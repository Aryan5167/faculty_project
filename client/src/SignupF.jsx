...
const SignupF = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios.post('http://localhost:3001/admin/register', { name, email, password ,dept,cabin}).then(result => console.log(result)).catch(err => console.log(err))
    }
        ...
        <div className="d-flex justify-content-center align-items-center bg-secondary vh-100 vw-100 " style={{width:"100%"}}>
            <div className="bg-white p-3 rounded w-50">
                ...
                <form onSubmit={handleSubmit} action="">
                    ...
                    <div className="mb-3">
                        <label htmlFor="dept"><strong> Department</strong>
                        </label>
                        <input type="dept" placeholder="Enter department" autoComplete="off" name="dept" className="form-control rounded-8" onChange={(e) => setDept(e.target.value)} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="cabin"><strong> Cabin Number</strong>
                        </label>
                        <input type="cabin" placeholder="Enter Cabin Number" autoComplete="off" name="cabin" className="form-control rounded-8" onChange={(e) => setCabin(e.target.value)} />
                    </div>
                    <button type="submit" className="btn btn-success w-100 rounded-0">Register</button>
                </form>
                <p>Already registered?</p>
                <Link to="/loginF" className="btn btn-default border w-100 bg-light rounded-0 text-decoration-none">Login</Link>
